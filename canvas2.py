...
coords = streamlit_image_coordinates(Image.fromarray(vis), key="bg_click_diff", width=800)
if coords:
    x = int(np.clip(coords["x"], 0, W-1))
    y = int(np.clip(coords["y"], 0, H-1))
    st.session_state.bg_points.append((x, y))

st.image(vis, caption="Originalbild mit Hintergrundpunkten", use_column_width=True)
...
if st.button("Differenzbild erzeugen") and st.session_state.bg_points:
    ...
    diff = cv2.subtract(gray, bg_mean)  # automatisch clipped 0–255
    ...
    combined = np.hstack([cv2.cvtColor(gray, cv2.COLOR_GRAY2RGB),
